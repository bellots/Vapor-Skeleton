...
public final class MyErrorMiddleware: Middleware, ServiceType {
    /// Structure of `MyErrorMiddleware` default response.
    internal struct ErrorResponse: Codable {
        ...
        var additionalDatas:String? = nil
    }
    
    /// See `ServiceType`.
    public static func makeService(for worker: Container) throws -> MyErrorMiddleware {
        return try .default(environment: worker.environment, log: worker.make())
    }
    
    /// Create a default `MyErrorMiddleware`. Logs errors to a `Logger` based on `Environment`
    ...
    ///     - log: Log destination.
    public static func `default`(environment: Environment, log: Logger) -> MyErrorMiddleware {
        return .init { req, error in
            ...
            do {
                let errorResponse = ErrorResponse(error: true, reason: reason, additionalDatas: nil)
    ...
    /// Create a new `MyErrorMiddleware`.
    ///
    /// - parameters:
